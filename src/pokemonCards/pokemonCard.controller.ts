...
export const postPokemonCards = async (req: Request, res: Response) => {
    ...
    res.status(201).send(`pokemonCard-POST ${req.body.name}`);
};
export const patchPokemonCards = async (req: Request, res: Response) => {
    ...
    if(getPokemonCardsId == prisma.pokemonCard.find("")){
        res.status(400).send("Error 400 bad Request - L'ID du pokemon est vide.");
        return 0;
    }
    if(req.body.name == ""){
        res.status(400).send("Error 400 Bad Request - Le nom du pokemon est vide.");
        return 0;
    }
